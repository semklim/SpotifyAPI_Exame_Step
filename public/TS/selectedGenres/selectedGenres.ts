...
function genPlaylistCards(list: object[]){
	...
	list.forEach((el) => {
		...
		html += `
		<section class="shelf" aria-label="${name}">
			<div class="shelf__control">
				<div class="shelf__name">
					<h2 draggable="false" class="shelf__name__title"  data-href="${href}">
...
function playlistByGenres(genres: string, obj: object[]): string{
	const html = `
